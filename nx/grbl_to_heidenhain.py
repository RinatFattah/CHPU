...
диалект крошечный, одиннадцать конструкций, и НИ ОДНОГО постоянного цикла:
    ...
    TOOL CALL S<обороты>
    M3 / M9 M5 / M30           конец: СОЖ и шпиндель гасятся одним блоком
...
def convert(src_lines, name="PROG", tol=0.01, z_datum="top"):
    ...
    # Конец программы: завод пишет ОДНИМ блоком `M9 M5` — СОЖ выключается
    # вместе со шпинделем, и так во всех шести файлах `PR_1_01…06`. Включения
    # (`M8`) нет ни у них, ни у нас, то есть M9 по существу ничего не гасит;
    # строка нужна для побуквенного совпадения служебной части с их
    # программами. Правится ПОСЛЕДНИЙ останов шпинделя, а не всякий: в
    # многоинструментальной программе M5 стоит и перед сменой инструмента.
    for i in range(len(out) - 1, max(-1, len(out) - 4), -1):
        if out[i] == "M5":
            out[i] = "M9 M5"
            break
    if out[-1] != "M30":
