...
`--no-finish`; мёртвые зоны (прижимы, границы стола; пока только с `--no-finish`) —
`--keepout-box X0 Y0 Z0 X1 Y1 Z1`, `--work-box …`, `--keepout-halfspace Z lt 5`,
`--keepout-margin MM` (см. [README_CAM.md](README_CAM.md#мёртвые-зоны-keep-out));
...
Инструмент `verify.py` проверяет, **насколько деталь после обработки отличается от
исходной** и укладывается ли отклонение в припуск. Результат симуляции (STL/STEP из
NX ISV или CAMotics) сравнивается с эталоном:

```bash
python run_cam.py деталь.step out.gcode --stock zag.step --verify-export
# → out.gcode прогнать в симуляторе, результат выгрузить как machined.step (или .stl)
python verify.py machined.step --from-export out --allowance 0.5
```

Эталон и маски пишутся в **STEP** (точное BREP, в СК G-кода); verify тесселлирует их
внутри через FreeCAD. Критерий: нет зареза ниже номинала И избыток над номиналом ≤
припуска (на достижимых поверхностях; зоны второго установа исключаются по маске).
Выход — сводка, цветная карта отклонений (`_deviation.ply`) и код возврата 0/2. Оба
входа — в системе координат G-кода (её и даёт `--verify-export`). Подробнее —
[README_CAM.md](README_CAM.md).
...
- Оснастка (прижимы) не моделируется геометрией, но защищается **мёртвыми зонами**
  (`--keepout-box` / `--work-box` / `--keepout-halfspace`) — пока только в черновой
  (`--no-finish`); материал в зоне остаётся несрезанным. Заготовка = габарит модели
  или файл (`--stock`).
