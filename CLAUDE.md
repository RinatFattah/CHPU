...
(1) `restrict_region` вычитает футпринт из 2D-зон Adaptive (grow=`zones["grow"]` —
Adaptive держит диск внутри региона; бокс участвует только если final_z ниже его
верха+m; в layers — по каждому диапазону);
(2) контур при зонах ВСЕГДА Adaptive-кольцом, выпущенным за край (вход сбоку из
воздуха, винтовой заход в узком поле не нужен — приём layers); Profile-петли у зон
ЗАПРЕЩЕНЫ: петля обводит внешний контур урезанной грани, т.е. режет вдоль стенок
выгрыза ВНУТРИ детали при формально чистом гейте (ловлено в NX на уголке —
траншеи). Сам Adaptive сводит заготовку к внешнему 2D-контуру (findShapeOutline),
всё вне его — расчищенный воздух, где вход и линки ходят НА ГЛУБИНЕ РЕЗА,
игнорируя вырез из региона (ловлено гейтом);
...
(5) ClearanceHeight ≥ верх боксов+m (`op_clearance`); (6) раздув ПОСТРОЕНИЯ зон =
m + ROUGH_TOLERANCE + 0.01 (`zones["grow"]`): слоп траектории Adaptive иначе
съедает зазор, и строгий гейт ловит касания на сотки. ГЛАВНЫЙ грабель:
