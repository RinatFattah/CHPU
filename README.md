...
# → out.gcode прогнать в симуляторе, результат выгрузить как machined.stl
python verify.py machined.stl --from-export out --allowance 0.5
```

Критерий: нет зареза ниже номинала И избыток над номиналом ≤ припуска (на достижимых
поверхностях; зоны второго установа исключаются по маске). Выход — сводка, цветная
карта отклонений (`_deviation.ply`) и код возврата 0/2. Оба входа — в системе координат
G-кода (её и даёт `--verify-export`). Подробнее — [README_CAM.md](README_CAM.md).
