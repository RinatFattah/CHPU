...
def simulate(gcode_path, stock_step_path, out_stem=None, nose_radius=0.4,
    ...
    # «Длина рабочей части» сверла НА САМОМ ДЕЛЕ РАЗДУВАЕТ ЕГО РАДИУС (замерено
    # на 4-13A, сверло Ø2.00 = r 1.000; отверстие в результате ISV):
    #     канавка 1 → r 1.411   2 → 2.232   3 → 3.157   то есть √(канавка² + r²)
    # Отсюда и старое «60 мм выметает деталь»: √(60² + r²) ≈ 60.
    # Ниже ~1 мм NX перестаёт учитывать заданный диаметр и делает ВЫРОЖДЕННЫЙ
    # инструмент ~Ø1.94: при канавке 0.5 и сверло Ø2, и сверло Ø10 дали одно и
    # то же r ≈ 0.968. Поэтому канавку держим на 1.0, а раздутие СНИМАЕМ,
    # подавая заниженный диаметр: d_модели = 2·√(r² − канавка²).
    # Второе ограничение: у NX есть ПРЕДЕЛ ДИАМЕТРА НА ДЛИНУ КАНАВКИ, d ≤ 3.88·L.
    # За пределом заданный диаметр игнорируется и инструмент схлопывается до
    # r ≈ 1.94·L. Все семь замеров ложатся в это правило:
    #   (Ø2, L0.5) → 0.968 = 1.94·L      (Ø10, L0.5) → 0.969 = 1.94·L
    #   (Ø2, L1)   → 1.411 = √(L²+r²)    (Ø10, L3)   → 5.817 = √(L²+r²)
    #   (Ø2, L2)   → 2.232               (Ø9.8, L1)  → 1.939 = 1.94·L
    #   (Ø2, L3)   → 3.157
    # Поэтому канавка НЕ константа, а доля диаметра: при L = 0.3·d обе величины
    # заведомо внутри предела, и компенсация даёт точный размер при любом сверле.
    frac = float(getattr(config, "NX_LATHE_DRILL_FLUTE_FRAC", 0.3))

    def _drill(d):
        """(диаметр для модели, длина канавки) — с поправкой на раздутие."""
        r = d / 2.0
        L = max(frac * d, 0.2)
        if L >= r:                      # компенсировать нечем, берём как есть
            return d, L
        return 2.0 * math.sqrt(r * r - L * L), L

    drills = []
    for t in tool_list:
        if t.get("type") not in (200, 220):
            continue
        dm, L = _drill(t.get("nose") or 0.0)
        drills.append({"n": t["n"], "diameter": dm, "flute": L})
