...
def mill(doc, feat, p, stock_solid=None):
    ...
    if p.get("_plan") is not None:
        ...
        # Шаги, которые УЖЕ решены во входном плане, в «невыполнено» не пишем:
        # иначе план агента утверждал бы, что шаг 15 не выполнен, хотя обороты
        # в нём назначены и обоснованы. Признак — обоснование с этим шагом.
        done = {g.get("шаг") for tr in (p.get("_plan_ops") or {}).values()
                for g in (tr.get("обоснование") or [])}
