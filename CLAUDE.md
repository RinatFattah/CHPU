...
### Постусловие шага и правило подачи (06.09.2026)

**У шага решения есть ПОСТУСЛОВИЕ** — детерминированная проверка его
собственного правила после патча, с отклонением ответа и повтором
(`agent.post_15`, `post_14`). От `plan_check` отличается областью: тот проверяет
план целиком и на любом входе (у плана-заготовки шаг 15 честно лежит в
«невыполнено», и это не нарушение), постусловие спрашивает, выполнил ли своё
правило шаг, который только что отработал. Повод — замер: в прогоне
`55_agent_all` шаг 15 сработал на 9 деталях из 12, у 025/027/031 сквозной вырез
остался на полных оборотах. Ошибка МОЛЧАЛИВАЯ: обороты не двигают инструмент,
воксельная сверка их не видит, машинное время ISV считается по подачам.
**Правило, которое проверяется арифметикой, незачем проверять моделью.**

**B12 — подача (уровень ЗАВОД).** Чистовой проход по КРИВОЛИНЕЙНОЙ грани идёт на
половинной подаче (1000 против 2000), всё остальное на полной. Признак — ПАРА
«вид + класс»: чистовой сам по себе не годится (`CAVITY_MILL_1_COPY`,
`PLANAR_MILL_1` идут на 2000), криволинейная грань сама по себе тоже
(`RADIYS_*_PR0.5` — черновые по тем же скруглениям — на 2000). Внутри чистового
прохода завод дробит дальше (шаг строчки 800, первый подвод 600) — у нас подача
задаётся на операцию, этого не выразить.

Две связанные правки: (1) **план врал о подаче** — проход по сечению скругления
печатает свою, а план записывал общую из инструмента; (2) правило применялось
только к ОДНОЙ из двух реализаций такого прохода — 3D-проход `Path.Op.Surface`
шёл на полной. Второе нашёл сам агент на детали 025. Теперь решение пишет
`_b12_feed` в план, а доводит код до него `apply_op_regimes` — и делает это уже
без входного плана, потому что у `Path.Op.Surface` своей подачи нет. Цена
измерена: траектория 025 побитово та же (без `F` и `S`), ISV 00:09:39.85 →
00:10:14.31.

**Замеры агентского слоя.** Прогон `58_polnyy_cikl` — полный цикл от модели по
всем 12 деталям, шаги 11/14/15: отказов ноль, приёмка нигде не хуже, две детали
улучшены слоем по B04 (005: 4 897 → 4 383 мм и 00:03:33.68 → 00:03:14.08 при
сверке 0/0; 025: 14 827 → 12 231 мм). Двукратных выигрышей нет нигде.

**Детектор бокового реза печатает объём беды**, а не только худшую точку: на 025
это «FinishSlope3, 1 кадр, 82 мм хода, до 22.1 мм вглубь». Разбор: строчки
3D-прохода на Y = −11.6 и −8.6 идут целиком снаружи детали, поверхности под
фрезой нет, проход опускается на дно (Z = 0.50) и едет по уровню полки сквозь
неочищенный угол заготовки высотой 22.6 мм. Единственная из 12 деталей.

**ПОЧИНЕНО** (`surface_boundary_enforce`, по умолчанию True): у `Path.Op.Surface`
две ручки — насколько РАСШИРИТЬ границу и СОБЛЮДАТЬ ли её, и использовались две
комбинации из четырёх. Было «расширить на радиус и не запирать»: край грани
покрыт, зато сканирование идёт по габариту ЗАГОТОВКИ. Стало «расширить И
соблюдать» — и покрытие края, и оседлость. Приёмка всех 12 деталей
(`61_granica_ryad` против `54`): десять не изменились ни на единицу, 023
4015 -> 3899 мм при сверке 0/0, 025 14827 -> 6245 мм и 00:09:39.85 ->
00:04:29.39, зарез 3.6 -> 0, недорез 34.6 -> 51.0, боковой рез исчез. Размен на
025 честный: раньше фреза доставала до стенки Y~0, выходя за границу грани, —
снимала больше, но местами резала В ДЕТАЛЬ; теперь оставляет плёнку 0.8 мм.
Стенка отклонена от вертикали на 2.373° и требует ватерлинии при любой границе.

**Итог сессии (`62_itog`, весь ряд, одна версия кода, агент в цепочке):** 005
4897 -> 4811 мм, 023 4015 -> 3899, 025 14827 -> 5941 мм и 00:09:39.85 ->
00:04:17.03 при зарезе 3.6 -> 0; девять прочих без изменений; отказов, внесённых
агентом, ноль. Шаги 14 и 15 воспроизводятся точно (постусловие), шаг 11 — нет:
у деления припуска нет единственного верного ответа, есть диапазон, и разброс
стоит до 10 % машинного времени. Снимается перебором с измеримой целевой
функцией (число рабочих ходов), не сделано.
