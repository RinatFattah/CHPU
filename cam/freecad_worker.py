...
def mill(doc, feat, p, stock_solid=None):
    ...
    if p.get("_plan") is not None:
        ...
        # Шаг считается выполненным, только если план НЕСЁТ ЕГО РЕЗУЛЬТАТ, а не
        # просто ссылку на блок. Наличия обоснования мало: у шага 3 оно есть и
        # дословно говорит «основание не с чертежа, квалитет и Ra неизвестны» —
        # то есть блок применён с умолчанием, а не выполнен. Поэтому список
        # узкий и явный: режим виден по значению, отличному от общего.
        _ops = (p.get("_plan_ops") or {}).values()
        done = set()
        if any(tr.get("обороты") and abs(float(tr["обороты"]) - rpm) > 1e-6
               for tr in _ops):
            done.add(15)
        if any(tr.get("подача") and abs(float(tr["подача"]) - feed) > 1e-6
               for tr in _ops):
            done.add(14)

        def _undone(шаг, *a, **kw):
            if шаг not in done:
                pl.undone(шаг, *a, **kw)

        _undone(3, "требования по квалитету и Ra не читаются: на моделях они "
                     "заданы цветом, расшифровки инструкции 30.0011.0155.998 нет",
                  "все грани обрабатываются как этап I")
        _undone(7, "порядок стадий зашит в код, а не выведен",
                  "объём → полоса → вырезы → грани сверху вниз → контур")
        _undone(13, "число зубьев фрезы неизвестно — в операционной карте не "
                      "указано")
        _undone(14, "подача не рассчитана по карте 3", f"F={feed:g} мм/мин из конфига")
        _undone(15, "скорость резания не рассчитана по карте 4",
                  f"n={rpm:g} об/мин из конфига")
        _undone(16, "проверка по силе и мощности не делается — нет паспорта станка")
    doc.recompute()
