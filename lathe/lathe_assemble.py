...
def section(body, z, r_split):
    ...
    ОТВЕРСТИЕ — ЭТО МИНИМУМ СЕЧЕНИЯ, а не «всё, что ниже порога r_split».
    Порог был знаменателен тем, что молча терял отверстие: у 10-11A он вставал
    на 4.09 при номинале отверстия 3.75 и минимальном наружном 4.43 — окно
    0.68 мм. В ISV отверстие выходит чуть больше номинала, стенка перескакивает
    порог, попадает в «наружные» точки, и сборка объявляла «отверстия нет» —
    сплошное тело вместо детали с каналом Ø7.5 (1590 мм³ фантомного недореза).

    Минимум/максимум порога не требуют: на любом z, где есть материал, сечение
    даёт замкнутое кольцо. У сплошного тела min == max (поверхность одна), у
    полого min — стенка канала. `wall` отделяет одно от другого.
    """
    RT, ZT, zlo, zhi = body
    m = (zlo <= z) & (zhi >= z)
    if not m.any():
    ...
    if rr.size == 0:
        return 0.0, 0.0
    r_hi, r_lo = float(rr.max()), float(rr.min())
    # стенка тоньше wall — считаем сечение сплошным: так у торцевых плоскостей,
    # где сечение ловит грань целиком, не выдумывается канал
    wall = 0.2
    bore = r_lo if (r_hi - r_lo) > wall and r_lo > 0.05 else 0.0
    return r_hi, bore
